...
function Dashboard() {
  const { currentUser, logout, t } = useStore();
  ...
  if (!currentUser || !currentUser.profileCompleted) return null;
      ...
      <header className="bg-primary text-primary-foreground sticky top-0 z-30 shadow-md">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium opacity-70">{t('appTitle')}</span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              <span className="text-sm font-medium hidden sm:block">{currentUser.name || currentUser.uniqueId}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-6 space-y-6">
        <div className="text-center py-2">
          <h1 className="text-3xl md:text-4xl font-bold text-foreground" style={{ fontFamily: 'var(--font-display)' }}>
            {currentUser.shopName || 'Complete Profile Setup'}
          </h1>
        ...
        <div className="flex items-center gap-4 py-2">
          ...
          <span className="text-sm font-semibold text-accent uppercase tracking-wider">{t('createYourBill')}</span>
          <div className="flex-1 h-px bg-border" />
        </div>

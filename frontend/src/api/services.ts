...
export async function apiLogin(uniqueId: string): Promise<LoginResponse> {
  const { data } = await api.post<LoginResponse>('/auth/login', { uniqueId });
  return data;
}

export async function apiSetupProfile(payload: {
  name: string;
  age: string;
  email: string;
  mobile: string;
  ...
  const { data } = await api.post<{ success: boolean; user: User }>('/auth/setup-profile', payload);
  return data.user;
}

...
export function BillingSection() {
  const {
    ...
    updateBillItemQty, clearBill, saveBill, currentUser, t,
  } = useStore();
    ...
    <div className="bg-card rounded-xl border border-border">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-foreground">{t('makeYourBill')}</h3>
        </div>
        {currentBillItems.length > 0 && (
          ...
          >
            <Trash2 className="w-3.5 h-3.5 mr-1" /> {t('clearBill')}
          </Button>
        )}
      </div>
      ...
      {showClearConfirm && (
        <div className="mx-4 mt-3 flex items-center justify-between bg-warning/10 border border-warning/30 rounded-lg p-3">
          <p className="text-sm text-foreground">{t('clearBillConfirm')}</p>
          <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => setShowClearConfirm(false)}>{t('cancel')}</Button>
            <Button size="sm" variant="destructive" onClick={() => { clearBill(); setShowClearConfirm(false); }}>{t('yesClearBill')}</Button>
          </div>
        </div>
      )}
      ...
      <div className="p-4 space-y-4">
        ...
        <div ref={searchRef} className="relative">
          <div className="relative">
            ...
            <Input
              placeholder={t('addToBillPlaceholder')}
              value={searchTerm}
        ...
        {currentBillItems.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="space-y-3">
              <p className="text-sm font-semibold text-foreground">{t('customerDetails')}</p>
              <Input value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder={t('fullName')} className="h-9" />
              <div className="grid grid-cols-2 gap-3">
                <Input value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} placeholder={t('phoneNumber')} className="h-9" />
                <Input value={customerAddress} onChange={e => setCustomerAddress(e.target.value)} placeholder={t('addressLabel')} className="h-9" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-2">
            ...
            <div className="bg-muted/30 rounded-lg p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">{t('cgst')} ({cgst || 0}%)</span>
                <span>₹{cgstAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">{t('sgst')} ({sgst || 0}%)</span>
                <span>₹{sgstAmount.toFixed(2)}</span>
              </div>
              <div className="border-t border-border pt-2 mt-2 flex justify-between">
                <span className="text-base font-bold text-foreground">{t('grandTotal')}</span>
              ...
              >
                {generating
                  ...
                  : <><FileText className="w-5 h-5 mr-2" /> {t('generateInvoice')}</>
                }
              </Button>
            </div>
          </div>
        ...
        {currentBillItems.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground text-sm">{t('searchAndSelectProducts')}</p>
          </div>
        )}
      </div>
    </div>

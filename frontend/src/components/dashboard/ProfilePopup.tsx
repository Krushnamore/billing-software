...
export function ProfilePopup({ isOpen, onClose, onLogout }: ProfilePopupProps) {
  const { currentUser, updateProfile, locale, setLocale, t } = useStore();
      ...
      <div className="profile-slide fixed inset-y-0 left-0 w-[80%] max-w-2xl bg-card z-50 shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="bg-muted/50 rounded-lg p-4 mb-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">{t('profileUniqueIdReadOnly')}</p>
              <p className="text-lg font-bold font-mono text-primary tracking-wider">{currentUser.uniqueId}</p>
            </div>
          </div>

          <div className="bg-muted/50 rounded-lg p-4 mb-6">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">{t('languageLabel')}</p>
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm font-semibold transition ${locale === option.value ? 'bg-secondary text-secondary-foreground' : 'bg-background text-foreground border border-border hover:bg-muted/70'}`}
                >
                  {option.label}
                </button>
              ))}
            </div>
            <p className="text-xs text-muted-foreground mt-3">{t('languageSelectionHint')}</p>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('personalInformation')}</p>
            ...
            <div className="border-t border-border pt-4 mt-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">{t('shopInformation')}</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t('gstNo')}</Label>
            ...
            <div className="border-t border-border pt-4 mt-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">{t('changePasswordOptional')}</p>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="flex gap-3 pt-4">
              <Button type="submit" className="flex-1 h-11" disabled={saving}>
                {saving
                  ...
                  : <><Save className="w-4 h-4 mr-2" />{saved ? t('saveSuccess') : t('saveChanges')}</>
                }
              </Button>
            </div>
          </form>
